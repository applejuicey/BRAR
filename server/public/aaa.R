# get the parameters from the rscript command line
args<-commandArgs(T)
...
maxium_sample_size <- as.numeric(args[1])
# ----------------------------------------------------------
# the allocation update frequncy:
allocation_update_frequncy <- as.numeric(args[5])
# ----------------------------------------------------------
# number of subjects to be assigned in the equal randomization stage
burn_in_period_length <- as.numeric(args[6])
# ----------------------------------------------------------
...
early_winner_cutoff <- as.numeric(args[2])
early_loser_cutoff <- as.numeric(args[3])
# TOBEDONE: ADD FULTILITY CRETERIA HERE
final_winner_cutoff <- as.numeric(args[4])
# ----------------------------------------------------------
# arm1 response rate:
arm1_response_rate <- as.numeric(args[7])
# arm2 response rate:
arm2_response_rate <- as.numeric(args[8])
# ----------------------------------------------------------

# PARAMETERS NEEDED IN THE OUTER ITERATION:
# ----------------------------------------------------------
# the beta prior parameters:
arm1_beta_prior_a <- as.numeric(args[9])
arm1_beta_prior_b <- as.numeric(args[10])
arm2_beta_prior_a <- as.numeric(args[11])
arm2_beta_prior_b <- as.numeric(args[12])
# ----------------------------------------------------------
# how many times the trial should be simulated:
trial_simulation_number <- as.numeric(args[13])
...
# ----------------------------------------------------------
# the random seed:
random_seed <- as.numeric(args[14])
...
for (j in 1:trial_simulation_number) {
  ...
  beta_param_vector_list[[1]][1] <- arm1_beta_prior_a
  beta_param_vector_list[[2]][1] <- arm1_beta_prior_b
  beta_param_vector_list[[3]][1] <- arm2_beta_prior_a
  beta_param_vector_list[[4]][1] <- arm2_beta_prior_b
  # ----------------------------------------------------------
  ...
  for (i in 1:length(iteration_vector)) {
    ...
    temp1 <- allocation_probability_solver1(inequaility_result_vector_list, burn_in_period_length + i - 1, maxium_sample_size, i)
...
# superior inferior ratio, supposed that arm2 is better
mean((arm2_positive_number_vector + arm2_negative_number_vector)/(arm1_positive_number_vector + arm1_negative_number_vector))
# mean total failures
mean(arm1_negative_number_vector + arm2_negative_number_vector)
sd(sample_size_vector)
